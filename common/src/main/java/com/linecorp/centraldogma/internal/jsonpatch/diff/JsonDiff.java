...
public final class JsonDiff
{
    ...
    public static JsonPatch asJsonPatch(final JsonNode source, final JsonNode target) {
        return asJsonPatch(source, target, ReplaceMode.RFC6902);
    }
    ...
    public static JsonPatch asJsonPatch(final JsonNode source,
        final JsonNode target, ReplaceMode replaceMode)
    {
        requireNonNull(source, "source");
        requireNonNull(target, "target");
        final Map<JsonPointer, JsonNode> unchanged
            = getUnchangedValues(source, target);
        final DiffProcessor processor = new DiffProcessor(unchanged, replaceMode);
    ...
    public static JsonNode asJson(final JsonNode source, final JsonNode target) {
        return asJson(source, target, ReplaceMode.RFC6902);
    }
     ...
     */
    public static JsonNode asJson(final JsonNode source, final JsonNode target, ReplaceMode replaceMode)
    {
        final String s;
        try {
            s = MAPPER.writeValueAsString(asJsonPatch(source, target, replaceMode));
        ...
        final ObjectNode target)
    {
        ...
        for (final String field: Sets.difference(firstFields, secondFields))
            processor.valueRemoved(pointer.append(JsonPointer.valueOf("/" + field)), source.get(field));
        ...
        for (final String field: Sets.intersection(firstFields, secondFields))
            generateDiffs(processor, pointer.append(JsonPointer.valueOf("/" + field)), source.get(field),
                target.get(field));
    }
        ...
        final ArrayNode target)
    {
        ...
        for (int index = size; index < firstSize; index++)
            processor.valueRemoved(pointer.append(JsonPointer.valueOf("/" + size)), source.get(index));

        for (int index = 0; index < size; index++)
            generateDiffs(processor, pointer.append(JsonPointer.valueOf("/" + index)), source.get(index),
                target.get(index));


        // Deal with the destination array being larger...
        JsonPointer endOfArrayPointer = JsonPointer.valueOf("/-");
        for (int index = size; index < secondSize; index++)
            processor.valueAdded(pointer.append(endOfArrayPointer), target.get(index));
    }
        ...
        final JsonNode target)
    {
        ...
        while (firstFields.hasNext()) {
            ...
            if (!target.has(name))
                continue;
            computeUnchanged(ret, pointer.append(JsonPointer.valueOf("/" + name)), source.get(name),
                target.get(name));
        }
    }
        ...
        final JsonPointer pointer, final JsonNode source, final JsonNode target)
    {
        ...
        for (int i = 0; i < size; i++)
            computeUnchanged(ret, pointer.append(JsonPointer.valueOf("/" + i)), source.get(i),
                target.get(i));
    }
}
