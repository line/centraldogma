...
const RepoCredentialViewPage = () => {
  const router = useRouter();
  const projectName = router.query.projectName as string;
  const repoName = router.query.repoName as string;
  const id = router.query.id as string;
  const { data, isLoading, error } = useGetRepoCredentialQuery({ projectName, id, repoName });
  return (
      ...
      {() => {
        return (
          <>
            ...
            <CredentialView projectName={projectName} repoName={repoName} credential={data} />
          </>
        );
      }}
    </Deferred>
...
export default RepoCredentialViewPage;
