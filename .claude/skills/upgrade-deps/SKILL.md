...
Central Dogma is a multi-module project with different minimum Java version requirements.
...
1. The flag assigned in `settings.gradle` (`java`, `java17`, etc.)
2. The project-level defaults in `gradle.properties` (`javaSourceCompatibility=11`, `javaTargetCompatibility=11`)

Resolution order in `gradle/scripts/lib/java.gradle` (`extractTargetJavaVersion()`):
- If the module has a `java17` (or similar) flag → that version wins
- Otherwise → falls back to `gradle.properties` values → **Java 11**
...
- Use the library's official name if one exists (e.g. `gRPC-Java`, `Jackson`, `Netty`, `Kotlin`,
  `Logback`, `Micrometer`); otherwise use the key name as-is from `dependencies.toml`
