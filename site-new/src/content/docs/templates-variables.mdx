...
description: Interpolate variables into configuration files and resolve them per project, repository or file.
---
...
- **YAML templates**: Variables may be unquoted only when YAML cannot interpret the value as another type.
  Quote values such as `false` to keep them as strings.
...
syntax. YAML templates accept unquoted values, but quote a STRING variable when YAML could interpret its value
as another type.
...
[Jackson 2.12 Most Wanted (4/5)](https://cowtowncoder.medium.com/jackson-2-12-most-wanted-4-5-cbc91c00bcd2).
...
![Variables list showing ID, type badges, modified by/at, and delete actions](./_images/variables-list.png)

### Creating and editing variables

Click "New Variable" or click on a variable ID to open the variable form:

![Variable form with ID, type selector, value editor, and description fields](./_images/variable-form.png)
...
![File editor showing Preview with Variables tab with rendered template output](./_images/preview-with-variables.png)
