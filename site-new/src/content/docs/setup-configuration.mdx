...
- `secret` (string)
  ...
  - the secret string which is used for replicas to authenticate each other. The replicas in the same
    ...
    random secret. You can secure this property by [hiding sensitive property values](#hiding-sensitive-property-values).
...
with your own `ConfigValueConverter` and register it via
