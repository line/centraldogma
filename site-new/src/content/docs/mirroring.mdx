---
title: Mirroring
description: Review configuration changes as pull requests and let Central Dogma mirror them from Git.
---
...
Modern source code hosting services such as [GitHub](https://github.com/) and [GitLab](https://about.gitlab.com/)
have a notion of [pull request](https://help.github.com/articles/about-pull-requests/). What if we use pull
...
![Setting up a mirroring task](./_images/mirroring_1.png)
...
- `Remote`
  ...
  - Use `git+http` only for public repositories that require no authentication. Never send a password or access
    token over `git+http`. Use `git+https` or `git+ssh` for authenticated repositories.
...
- `Zone` (Displayed only if `zone` in the [configuration](setup-configuration.mdx) is configured)
...
- `Remote`
  ...
  - Use `dogma+https` outside localhost or local development. When an access token is configured, never send it
    over `dogma`.
...
- `Credential`
  ...
  - the optional ID of an access token credential for a remote Central Dogma server that requires authentication.
    Use the credential only with `dogma+https`.
...
As your configuration grows, you may want to bump the limit. See [the configuration guide](setup-configuration.mdx)
to learn about the options related with mirroring: `numMirroringThreads`, `maxNumFilesPerMirror` and
