---
title: xDS control plane
description: Serve Envoy configuration from version-controlled YAML with Central Dogma's xDS control plane.
---

# Serving Envoy configuration with the xDS control plane

Central Dogma can act as an [Envoy](https://www.envoyproxy.io/)
[xDS](https://www.envoyproxy.io/docs/envoy/latest/api-docs/xds_protocol) v3 control plane. It stores Envoy
...
[configuration change workflow](mirroring.mdx) and [access control](auth.mdx) you already use for your
...
![xDS group list](./_images/xds_1.png)
...
![Creating a new xDS group](./_images/xds_2.png)

### Editing resources

Selecting a group reveals a sidebar that navigates everything in it:

- _Overview_ — a summary of the group.
- _Listeners_, _Routes_, _Clusters_, _Endpoints_ — the four core resource types.
- _K8s Aggregators_ — generate Endpoints from Kubernetes services (see
  [Aggregating Kubernetes service endpoints](#aggregating-kubernetes-service-endpoints)).
- _References_ — a dependency graph of the group's resources, with reverse-reference lookup and
  dangling-reference detection.
- _History_ — the commit log of every resource change.
- _Mirroring_, _Credentials_, _Permissions_, _Danger Zone_ — administrative sections, visible to group
  administrators only.
...
[Listener](https://www.envoyproxy.io/docs/envoy/latest/api-v3/config/listener/v3/listener.proto),
[RouteConfiguration](https://www.envoyproxy.io/docs/envoy/latest/api-v3/config/route/v3/route.proto),
[Cluster](https://www.envoyproxy.io/docs/envoy/latest/api-v3/config/cluster/v3/cluster.proto) and
[ClusterLoadAssignment](https://www.envoyproxy.io/docs/envoy/latest/api-v3/config/endpoint/v3/endpoint.proto)
messages.

![Editing an xDS resource](./_images/xds_3.png)

The _Mirroring_ section configures [mirroring](mirroring.mdx) for the group's backing repository, the
_Permissions_ section manages [access control](auth.mdx), and the _Credentials_ section holds the credentials
...
Open a group's _K8s Aggregators_ section and click "New". The editor is a form: enter an _Aggregator ID_, then
add one or more _Watchers_ — each watcher maps to a single Kubernetes service. Click "Preview endpoints" to
...
read-only, in the group's _Endpoints_ section.

![Creating a Kubernetes endpoint aggregator](./_images/xds_4.png)
...
- `Credential ID` (`kubeconfig.credentialId`)
  ...
  - the ID of an access-token credential in the group's _Credentials_ section, used as the OAuth token to
    authenticate to the Kubernetes API server. See [authentication](auth.mdx) for credential management.

- `Trust all certificates (skip verification)` (`kubeconfig.trustCerts`)

  - trusts all TLS certificates without verification. Use this only in a controlled environment with a self-signed
    control plane.
    An intercepted connection could expose the configured OAuth token and return falsified endpoint data.
...
Application identities menu. To give it access to a group, register it under that group's _Permissions_
section, in _Application IDs_, with the desired role; _Permissions_ grants roles to individual users the same
way. See [authentication and access control](auth.mdx) for the underlying authentication and access-control model.

The _Credentials_ section is unrelated to granting clients access. It holds the credentials the group uses
itself: to [mirror](mirroring.mdx) its backing repository — with an SSH key, a password or an access token —
and to authenticate to a Kubernetes control plane, which requires an access token (the _Credential ID_ a
watcher references).
...
Run the xDS control plane with [authentication](auth.mdx) enabled and authenticate every xDS client, so
